...
SELECT * FROM fireIncidents
WHERE `Incident Date` LIKE '04/04%' OR `Incident Date` LIKE '27/09%'
ORDER BY `Incident Number` ASC
...
SELECT * FROM fireIncidents
WHERE (`Incident Date` LIKE '04/04%' OR `Incident Date` LIKE '27/09%') AND `Station Area` > 20
ORDER BY `Incident Number` ASC
...
SELECT COUNT(*) AS incidentNumber
FROM fireIncidents
WHERE `Incident Date` LIKE '04/04/2016'
...
SELECT `Ignition Cause`,COUNT(`Incident Number`) AS incidentNumber
FROM fireIncidents
WHERE `Ignition Cause` LIKE '4 act of nature'
...
FROM fireCalls AS fc
INNER JOIN fireIncidents AS fi
